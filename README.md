...
- **Delegação sob demanda (X1)**: `config/dados/delegados.json` cataloga **5 subagentes** — `delegar_pesquisa`, `delegar_redacao`, `delegar_codigo` (implementa com testes no sandbox de escrita), `delegar_dados` (análise com pandas) e `delegar_revisao` (segunda opinião G3 sob demanda) — cada um com pool de ferramentas restrito (resolvido por nome) e `arq_limite` anti-cascata (delegação aninhada bloqueada no limite).
- **Skills/playbooks (X2)**: `extensions/skills/<nome>/SKILL.md` com frontmatter (nome, descrição, **gatilho**) vira **memória procedimental versionada** — a tool `carregar_skill` lista o catálogo por descrição e injeta o corpo no contexto sob demanda (teto `AEGIS_SKILL_TETO_TOKENS`, truncamento sinalizado). Registro dinâmico sem reiniciar, skill inválida ignorada com aviso, RAG-lite ranqueia pela descrição. Skills versionadas: `pesquisa-tecnica` e `revisar-codigo`.
- **Fact-checking com fontes (X3)**: `buscar_web` devolve `[{url, titulo, trecho}]` estruturado e o nó `no_fact_check` (zero-LLM) cruza ≥2 fontes por consulta antes do fim do turno — consistência → `afirmado` com URLs na resposta; conflito → `divergencia` sinalizada ("fontes divergem — url1 vs url2"). Estado auditável `fontes: [{afirmacao, urls, status}]`; turno sem web não paga o custo.
...
| `AEGIS_SKILL_TETO_TOKENS` | `4000` | Teto de tokens ao injetar o corpo de uma skill no contexto via `carregar_skill` (X2) |
